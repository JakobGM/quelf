...
for (x in 1:45) {
  print(x)
grade_statistics <- grades %>% slice(x) %>%
...
grades[36,]$course_code <- "TDT05"
grade_statistics[36,]$course_code <- "TDT05"
grades[37,]$course_code <- "TDT17"
grade_statistics[37,]$course_code <- "TDT17"
...
grades %>%
  ...
  geom_text(
    aes(
      y = 37 - 0.5 * total - cumsum(c(0, total[1:3])),
      x = 1,
...
grade_cummulatives %>%
  ...
  annotate("text", y = 1.15, x = 38, label = "Time Investment", color = "gray") +
  xlab("") +
  ylab("") +
  coord_flip() +
