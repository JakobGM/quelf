...
```{r, cache = TRUE}
# Read in Toggl database, produced with the following CLI command:
# toggl-to-sqlite fetch data/toggl.db --since 2014-01-01
db <- DBI::dbConnect(drv = RSQLite::SQLite(), dbname = "../data/toggl.db")

# The three tables present in the Toggl database
time_entries_table <- tbl(db, "time_entries")
projects_table <- tbl(db, "projects")
workspaces_table <- tbl(db, "workspaces")

# Join the three tables together and only retrieve university time entries
toggl <- time_entries_table %>%
  left_join(projects_table, by = c("pid" = "id"), suffix = c("", ".p")) %>%
  left_join(workspaces_table, by = c("wid" = "id"), suffix = c("", ".w")) %>%
  filter(name.w == "NTNU") %>%
  select(
    course = "name",
    description,
  ...
  mutate(
    date = DATE(start),
    study_session_duration = sql("24 * (julianday(end) - julianday(start))")
  ) %>%
  filter(study_session_duration < 12) %>%
  as_tibble() %>%
  mutate(
    start = ymd_hms(start),
    end = ymd_hms(end),
    date = ymd(date),
    is_course = (
      course != "TMA4900: Masteroppgave"
      & course != "TMA4500: Fordypningsprosjekt"
    )
  )
...
```{r, fig.height = 4}
events <- tibble(
  date = c(date("2016-01-15"), date("2020-03-12")),
  label = c("First RSI\nSympton", "COVID-19\nLockdown")
)
sick_leave <- tibble(
...
grades <- tibble(
    course = toggl %>%
      ...
      unlist(),
    course_code = c(
      "EXPH0004", "MFEL3010", "Org", "TDT05", "TDT17", "TDT4102", "TDT4105",
      "TDT4120", "TDT4136", "TDT4145", "TDT4173", "TEP4105", "TFY4145",
      "TFY4155", "TFY4165", "TFY4215", "TIØ4146", "TIØ4258", "TMA4100",
      "TMA4105", "TMA4115", "TMA4120", "TMA4145", "TMA4150", "TMA4180",
      "TMA4195", "TMA4212", "TMA4215", "TMA4245", "TMA4265", "TMA4267",
      "TMA4268", "TMA4295", "TMA4300", "TMA4315", "TMA4320", "TMA4500",
      "TMA4850", "TMA4900", "TMT4110"
    ),
    course_name = c(
      "Science, Philosophy & Ethics",
      "Medicine for Technologists",
      "Organization",
    ...
    grade = as.ordered(c(
      "A", "A", NA, "A", "A", "A", "A", "A", "B", "B", NA, "B", "A", "A", "B",
      "B", "A", "B", "A", "A", "A", "B", "C", NA, "C", "C", "C", "C", "A", "D",
      "B", "A", "C", "B", "A", "B", "A", "A", "A", "B"
    )),
    semester = c(
      1, 11, NA, 11, 11, 4, 1, 5, 11, 10, 10, 3, 1, 2, 3, 10, 9, 3, 1, 2, 2,
      3, 5, 6, 6, 9, 6, 9, 4, 5, 6, 10, 5, 10, 9, 4, 11, 10, 12, 2
    )
  ) %>%
  arrange(semester) %>%
...
grades[35,]$course_code <- "TDT4506"
grades[36,]$course_code <- "TDT4506"

# Master thesis statistics are not available fro V2020 as of this writing
grades[39,]$semester_code <- "V2019"

# Add the grade statistics
grade_statistics <- grades %>%
  filter(course_code != "Org") %>%
  rowwise() %>%
  do(row = fetch_grade_statistics(as.list(.))) %>%
  use_series(row) %>%
  bind_rows()

# Reintroduce correct course codes in order to make the left_join below correct
grades[35,]$course_code <- "TDT05"
grade_statistics[35,]$course_code <- "TDT05"
grades[36,]$course_code <- "TDT17"
grade_statistics[36,]$course_code <- "TDT17"

grades %<>% left_join(grade_statistics, by = "course_code")

# Rename back to original course codes for the physics courses as well
grades[3,]$course_code <- "TFY4145"
grades[5,]$course_code <- "TFY4155"

# And, at last, reintroduce the correct semester code for the Master's thesis
grades[39,]$semester_code <- "V2020"

# Insert column representing ECTS points of each course
# At NTNU there are usually 4 courses each semester with 7.5 ECTS points each,
# but with some exceptions...
grades %<>%
  mutate(
    ects_points = case_when(
      course_code == "TMA4900" ~ 30,
      course_code == "TMA4500" ~ 15,
      course_code == "TDT05" ~ 3.75,
      course_code == "TDT17" ~ 3.75,
      course_code == "Org" ~ 0,
      TRUE ~ 7.5
    ),
    ects_points = na_if(ects_points, 0),
    course_equivalents = ects_points / 7.5
  )
...
grades %>%
  ...
  geom_text(
    aes(
      y = sum(total) - 0.5 * total - cumsum(c(0, total[1:3])),
      x = 1,
...
courses %>%
  drop_na(grade) %>%
  ggplot() +
  aes(x = reorder(course_name, -study_hours / course_equivalents), y = study_hours / course_equivalents) +
  ...
  scale_hours(y = TRUE) +
  labs(
    y = "Time Spent on Course per Grade*",
    x = "",
    caption = expression(italic('*The time spent on "Master Thesis" and "Specialization Project" has been divided by 4 and 2 respectively; the number of courses they equate to.'))
  ) +
  theme(
...
grade_intervals <- tibble(
  ...
  inner_join(
    courses %>% select(course_name, study_hours, course_equivalents),
    by = "course_name"
  )

# TODO: Find out what this logic did, since it broke when updated with new data
...
grade_cummulatives %>%
  ...
  ggplot() +
  aes(x = reorder(course_name, -study_hours / course_equivalents)) +
  geom_linerange(
  ...
  geom_linerange(
    aes(
      ymin = 1.035,
      ymax = 1.035 + (1 / 700) * study_hours / course_equivalents
    ),
    size = 0.4 * size,
    alpha = 0.7,
    color = "gray"
  ...
  annotate("text", y = 1.2, x = 38.5, label = "Time Investment\nper Grade", color = "gray") +
  xlab("") +
  ylab("") +
  coord_flip() +
...
courses %>%
  ...
  filter(!is.na(grade)) %>%
  ggplot() +
  aes(x = reorder(course_code, semester), y = study_hours / course_equivalents) +
  geom_bar(stat = "identity", aes(fill = grade)) +
  geom_vline(
...
class_size <- toggl %>%
  ...
  select(-a, -b, -c, -d, -e, -f, -study_hours, -grade, -course_name, -semester, -course_equivalents) %>%
...
class_size %>%
  ggplot() +
  aes(x = class_size, y = study_hours / course_equivalents, color = grade) +
  geom_point(size = 6) +
  geom_text(
  ...
  labs(
    ...
    y = "Total Study Hours / Course Equivalents",
